...
/// <summary>
/// Skill ของ Ice: ยิงก้อนน้ำแข็งพุ่งไปข้างหน้า ทำดาเมจและชะลอศัตรูที่โดน
/// แนบสคริปต์นี้ไว้ที่ Prefab ของ Skill Ice แล้วลากใส่ช่อง "Item Skill" ใน ItemData
/// </summary>
public class IceSkill : BaseItemSkill
{
    ...
    public override void Activate(Transform playerTransform)
    {
        ...
        Debug.Log($"<color=#88DDFF>[IceSkill] ยิงก้อนน้ำแข็ง! ดาเมจ: {damage}, ชะลอ: {slowPercent * 100}% เป็นเวลา {slowDuration} วินาที</color>");
    }
    ...
    private void Update()
    {
        ...
        if (Physics.Raycast(transform.position, transform.forward, out RaycastHit hit, moveDistance + 0.2f))
        {
            if (hit.collider.CompareTag("Enemy"))
            {
                ...
                Debug.Log($"<color=#88DDFF>[IceSkill] โดนศัตรู {hit.collider.name}! สร้างความเสียหาย {damage} + ชะลอ</color>");
